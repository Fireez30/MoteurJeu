...
void Room::ReadFile(std::vector<Rooms>* r,int index, std::string path, Player* p, Camera* c){
    ...
    for (tinyxml2::XMLElement* e2 = d2->FirstChildElement("tile"); e2 != nullptr; e2 = e2->NextSiblingElement("tile")){//y
        Tile t= Tile(QVector2D((float)e2->IntAttribute("x")+xRoom,(float)(-1*e2->IntAttribute("y"))+yRoom),QVector2D(e2->IntAttribute("tx")/16.0,e2->IntAttribute("ty")/16.0));
        tiles.push_back(t);
    ...
    if (d4){//si la salle que l'on lit contient une entité
        ...
        for (tinyxml2::XMLElement* e4 = d4->FirstChildElement("RangedPile"); e4 != nullptr; e4 = e4->NextSiblingElement("RangedPile")){//Liste des piles
            RangedPile *r = new RangedPile(QVector2D(e4->IntAttribute("x")/16.0+xRoom,(-1*e4->IntAttribute("y")/16.0)+yRoom));
        ...
        for (tinyxml2::XMLElement* e4 = d4->FirstChildElement("LargerPile"); e4 != nullptr; e4 = e4->NextSiblingElement("LargerPile")){//Liste des piles
            LargerPile *r = new LargerPile(QVector2D(e4->IntAttribute("x")/16.0+xRoom,(-1*e4->IntAttribute("y")/16.0)+yRoom));
            r->setCollider(Hitbox(QVector2D(r->position.x(),r->position.y()),1,1));
            pickups.push_back(r);
        }
        for (tinyxml2::XMLElement* e5 = d4->FirstChildElement("Ghost"); e5 != nullptr; e5 = e5->NextSiblingElement("Ghost")){//Liste des Ghosts
            Ghost* e =new Ghost(this,p,QVector2D(e5->IntAttribute("x")/16.0+xRoom,(-1*e5->IntAttribute("y")/16.0)+yRoom));
        ...
        for (tinyxml2::XMLElement* e5 = d4->FirstChildElement("TurretEnnemi"); e5 != nullptr; e5 = e5->NextSiblingElement("TurretEnnemi")){//Liste des Ghosts
            ...
            TurretEnnemi* e =new TurretEnnemi(this,p,QVector2D(e5->IntAttribute("x")/16.0+xRoom,(-1*e5->IntAttribute("y")/16.0)+yRoom),QVector2D(e5->IntAttribute("directionx"),e5->IntAttribute("directiony")),(bool)tir);
            e->setCollider(Hitbox(QVector2D(e->position.x(),e->position.y()),1,1));
            entities.push_back(e);
        }
        ...
        for (tinyxml2::XMLElement* e5 = d4->FirstChildElement("Boss_torche"); e5 != nullptr; e5 = e5->NextSiblingElement("Boss_torche")){//Liste des Boss_torche
            Boss_torche* e =new Boss_torche(this,p,QVector2D(e5->IntAttribute("x")/16.0+xRoom,(-1*e5->IntAttribute("y")/16.0)+yRoom));
            e->setCollider(Hitbox(QVector2D(e->position.x(),e->position.y()),1,1));
            entities.push_back(e);
            boss2 = e;
        }//fin for piles, rajouter des fors pour les autres entités
        ...
        if(pickups[i]->canCollide && pickups[i]->getCollider().TestCollision(other->getCollider())){
            ...
            if(pile != nullptr && p != nullptr){
                ...
                if(pileJoueur!= nullptr && dynamic_cast<RangedPile*>(pileJoueur)){
                    RangedPile *r = new RangedPile(QVector2D(pile->position.x(),pile->position.y()),pileJoueur->GetRange(),pileJoueur->GetConeAngle(),pileJoueur->getLifespan()/60,pileJoueur->getMaxLifespan()/60,pileJoueur->GetDamage(),pileJoueur->renderer.GetTextCoords());
                    r->setCollider(Hitbox(QVector2D(r->position.x(),r->position.y()),1,1));
                    ...
                    Pile * r = new LargerPile(QVector2D(pile->position.x(),pile->position.y()),pileJoueur->GetRange(),pileJoueur->GetConeAngle(),pileJoueur->getLifespan()/60,pileJoueur->getMaxLifespan()/60,pileJoueur->GetDamage(),pileJoueur->renderer.GetTextCoords());
                    r->setCollider(Hitbox(QVector2D(r->position.x(),r->position.y()),1,1));
                    r->renderer.CreateGeometry();
                    r->canCollide = false;
                    r->startTimer();
